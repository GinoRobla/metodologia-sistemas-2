...
function MisTurnosPage() {
  ...
  useEffect(() => {
    // 1. Evita llamadas a la API si el token es nulo al inicio
    //    Esto permite que useAuth primero intente cargarlo desde localStorage
    if (!token && user === null) {
        setIsLoading(false);
        return; 
    }
    ...
    const cargarMisTurnos = async () => {
        setIsLoading(true);
        try {
            // El interceptor en api.ts automáticamente añade el token al header 'Authorization'
            const response = await api.get('/turnos/mis-turnos'); 
            setTurnos(response.data);
            setError('');
        } catch (err: any) {
            console.error('Error al cargar turnos:', err);
            // El interceptor ya maneja la redirección si es un 401
    ...
    // 2. Dependencia clave: Recargar cuando el token cambie (ej. al iniciar sesión)
}, [token, user]);
  ...
  const handleCancelar = async (id: string) => {
    ...
    try {
        // El interceptor en api.ts añade el token automáticamente
